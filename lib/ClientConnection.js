...
class ClientConnection {
    ...
    constructor(server, connectionId, webSocket) {
        ...
        this.clientOptions.log.stdLog(`New connection initiated on ${this.webSocket.url} or upgraded ${this.webSocket.upgradeReq.url}`);
        this.query = Url.parse(this.webSocket.upgradeReq.url, true).query;
        this.clientOptions.log.stdLog(`Query parsed as ${this.query}`);
        this.lastActivity = Date.now();
        this.activityCheckInterval = null;

        this.log(this.server.LOGLEVEL.VERBOSE, 'Client connection open');
        this.clientOptions.log.stdLog(`Client connection open?`);
